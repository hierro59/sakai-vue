        ...
        >
            ...
            <Column :exportable="false" style="min-width: 12rem">
                <template #body="slotProps">
                    ...
                    <Button icon="pi pi-trash" outlined rounded severity="danger" class="mr-2" @click="confirmDeleteMember(slotProps.data)" v-if="slotProps.data.status !== 'inactive'" v-tooltip.top="'Eliminar de la comunidad'" />
                </template>
            </Column>
        </DataTable>
...
const saveMembers = () => {
    ...
    api.addCommunityMembers(props.communityId, selectedMembers.value)
        .then((response) => {
...
const deleteMember = () => {
    api.deleteCommunityMembers(props.communityId, [user.value.id])
        .then((response) => {
            console.log(response);
...
const deleteSelectedUsers = async () => {
    ...
    try {
        await api.deleteCommunityMembers(props.communityId, ids);
...
const getUsers = () => {
    api.getUsers(props.communityId)
        .then((response) => {
